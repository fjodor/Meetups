...
## 2. Multi-Language and Multi-Engine
...
- No R dependency
- Quarto highly likely to become new standard
- You may encounter collaborators working in other languages sooner than you expect
- Quarto more easily used from other software; can be called from command line
- supports editors beyond RStudio like JupyterLab, VS Code
:::
:::
...
- designed to work with languages that don't even exist today


## 3. Small Cost of Switching

- Depending on your formats, some adaption may be necessary
- Benefit: More consistency
  + easier to switch between output formats, e. g. from book to website or slides
  + Markdown: `_bookdown.yml`, `_output.yml` etc.
  + Quarto: Just one single `_quarto.yml`


## My Experience & More

::: {.incremental style="font-size: 2rem"}
- Presentations: switched from **rmarkdown & xaringan** to **quarto & revealjs**
- Layout more flexible, e. g. adjust widths for 2-or-more column layouts
  + xaringan: edit .css file
  + quarto & revealjs: `::: {.column width="65%"}`
- Tabsets work across different html output formats <br>(but can be a hassle for PDF printing)
- Easier to set global chunk options
  + RMarkdown: `knitr::opts_chunk$set(echo = FALSE)` in R code block
  + Quarto: `execute: echo: false` in YAML header
- Show code chunks including chunk options: use `echo: fenced`
- New features may only be implemented in Quarto!
:::

::: {.notes}
What is NOT implemented in Quarto?

- paged HTML: pagedown for rmarkdown; planned for Quarto 
- flexdashboard: planned for Quarto 
- interactive learnr tutorials: no equivalent planned!
...
- Documentation: **Quarto website** at  
  https://quarto.org/
